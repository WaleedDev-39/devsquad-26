...
const TMP_TASKS_FILE = path.join(os.tmpdir(), "tasks.json");

let cachedTasks = null;

// Helper to read tasks. Prioritizes memory cache, then /tmp, then bundled file.
const readTasks = () => {
  if (cachedTasks) return cachedTasks;

  try {
    // Try reading from /tmp first as it might contain updates
    if (fs.existsSync(TMP_TASKS_FILE)) {
      const tmpData = fs.readFileSync(TMP_TASKS_FILE, "utf-8");
      cachedTasks = JSON.parse(tmpData);
      return cachedTasks;
    }
  } catch (err) {
    console.error("Error reading from temp file:", err.message);
  }

  try {
    // Fallback to the bundled tasks.json
    const data = fs.readFileSync(TASKS_FILE, "utf-8");
    cachedTasks = JSON.parse(data);
    return cachedTasks;
  } catch (error) {
    console.error("Error reading from project file:", error.message);
    return [];
  }
};

// Helper to write tasks. Updates memory cache and attempts file persistence.
const writeTasks = (tasks) => {
  cachedTasks = tasks;
  const data = JSON.stringify(tasks, null, 2);

  try {
    // Attempt writing to project file (succeeds locally)
    fs.writeFileSync(TASKS_FILE, data, "utf-8");
  } catch (error) {
    // Fallback to /tmp (needed for Vercel's read-only filesystem)
    try {
      fs.writeFileSync(TMP_TASKS_FILE, data, "utf-8");
    } catch (err) {
      console.error("Persistence failed: Could not write to /tmp", err.message);
    }
  }
};
