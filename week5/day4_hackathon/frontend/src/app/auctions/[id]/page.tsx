          ...
          <div className="lg:w-3/4">
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              ...
              <div className="flex-1 relative h-[400px] md:h-[500px] rounded overflow-hidden bg-gray-100">
                ...
                <Image 
                  src={car.photos && car.photos[activeImage] ? (car.photos[activeImage].startsWith('http') ? car.photos[activeImage] : `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}${car.photos[activeImage].startsWith('/') ? '' : '/'}${car.photos[activeImage]}`) : '/placeholder-car.jpg'} 
                  alt={car.title} 
                  fill 
                  unoptimized
                ...
                {[...Array(6)].map((_, idx) => (
                  <div key={idx} className="relative h-[120px] md:h-auto rounded overflow-hidden bg-gray-100 cursor-pointer">
                    {car.photos && car.photos[idx] ? (
                      <Image 
                        src={car.photos[idx].startsWith('http') ? car.photos[idx] : `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}${car.photos[idx].startsWith('/') ? '' : '/'}${car.photos[idx]}`} 
                        alt="" 
                        fill 
                        unoptimized
